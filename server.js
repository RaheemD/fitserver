// server.js - paste & redeploy (full replacement)

const express = require("express");
const http = require("http");
const process = require("process");

// fetch + AbortController compatibility
let fetchImpl = global.fetch;
let AbortControllerImpl = global.AbortController;
try { if (!fetchImpl) fetchImpl = require("node-fetch"); } catch (e) {}
try { if (!AbortControllerImpl) AbortControllerImpl = require("abort-controller"); } catch (e) {}
...
// Allowed origins (add your production frontends here)
...
// CORS middleware: echo requested headers to avoid preflight header rejection
app.use((req, res, next) => {
  ...
  if (origin && ALLOWED_ORIGINS.includes(origin)) {
    ...
    // For debugging/local, allow any origin - tighten this in production
    res.setHeader("Access-Control-Allow-Origin", "*");
  }

  res.setHeader("Access-Control-Allow-Methods", "GET,POST,OPTIONS,PUT,DELETE");
  // If the browser sends Access-Control-Request-Headers, echo them back so preflight succeeds
...
async function fetchWithTimeoutAndRetry(url, options = {}, timeoutMs = 60000, maxRetries = 1) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      const resp = await (fetchImpl || global.fetch)(url, { ...options, signal });
      if (timer) clearTimeout(timer);
      if (resp.status >= 500 && attempt < maxRetries) {
        lastErr = new Error(`Upstream ${resp.status} (retrying)`);
      ...
      if (attempt < maxRetries) {
        console.warn(new Date().toISOString(), "Fetch failed, retrying:", err && err.message);
        await new Promise((r) => setTimeout(r, 300 * (attempt + 1)));
        continue;
      }
      throw lastErr;
...
app.post("/api/myapi", async (req, res) => {
  ...
  try {
    ...
    console.log("Upstream preview:", upstreamText ? upstreamText.slice(0, 1200) : "(empty)");

    // If upstream returned HTML (starts with '<'), convert to a JSON error so client won't crash
